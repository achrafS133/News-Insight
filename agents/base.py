...
class BaseRagAgent(ABC):
    ...
    def _call_llm(self, messages: list[dict[str, str]], *, trace_name: str) -> str:
        # If OpenAI API key is not set, return a stubbed response so the app can run offline
        key = self.settings.openai_api_key.get_secret_value()
        if not key or not key.strip():
            return f"LLM stub: openai_api_key not configured for agent {self.agent_name}"
