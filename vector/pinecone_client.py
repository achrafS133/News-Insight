...
class PineconeVectorStore:
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        if _HAS_PINECONE:
            ...
            # In-memory fallback store: list of (id, vector, metadata)
            self._client = None
            self._index = None
            self._store: List[Tuple[str, List[float], dict[str, Any]]] = []
    ...
    def upsert_batch(
        ...
        *,
        ...
        with LangfuseTraceContext(
            ...
            # In-memory fallback: append to local store
            for vid, values, meta in vectors:
                self._store.append((vid, values, meta or {}))
            flush_langfuse()
            return len(vectors)
    ...
    def query(
        ...
        *,
        ...
        with LangfuseTraceContext(
            ...
            # In-memory simple cosine similarity search
            ...
            for vid, vec, meta in self._store:
                if language and meta.get("language") != language:
                    continue
                # cosine similarity
                denom = _norm(vec) * _norm(embedding)
                score = _dot(vec, embedding) / denom if denom else 0.0
                results.append((score, vid, meta))

            results.sort(key=lambda x: x[0], reverse=True)
            hits: list[RetrievalHit] = []
            for score, vid, meta in results[: top_k or self.settings.retrieval_top_k]:
                hits.append(
                    RetrievalHit(
                        chunk_id=str(meta.get("chunk_id", vid)),
                        score=float(score),
                        chunk_text=str(meta.get("chunk_text", "")),
                        metadata=dict(meta),
                    )
